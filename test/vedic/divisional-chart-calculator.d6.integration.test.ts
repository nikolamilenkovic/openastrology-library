...
describe(DivisionalChartCalculator.name, () => {
    ...
    describe(D + ' calculations for test person 1', () => {
        ...
        it('should calculate ' + D + ' planet signs successfully', () => {
            ...
            expect(result.planets.mercury.sign).toBe('sagittarius');
            expect(result.planets.jupiter.sign).toBe('leo');
        ...
        it('should calculate ' + D + ' ascendant degree correctly', () => {
            ...
            expect(result.ascendant.degree).toBeWithinEpsilon(dms2def('12:18:21'), 0.1);
        });

        it('should calculate ' + D + ' planet degrees correctly', () => {
            const result = chartCalculator.calculateDivisionalChart(rashiChart, D);

            expect(result.planets.sun.degree).toBeWithinEpsilon(dms2def('03:36:27'), 0.1);
            expect(result.planets.moon.degree).toBeWithinEpsilon(dms2def('26:32:01'), 0.1);
            expect(result.planets.mars.degree).toBeWithinEpsilon(dms2def('14:46:58'), 0.1);
            expect(result.planets.mercury.degree).toBeWithinEpsilon(dms2def('13:55:53'), 0.1);
            expect(result.planets.jupiter.degree).toBeWithinEpsilon(dms2def('13:14:39'), 0.1);
            expect(result.planets.venus.degree).toBeWithinEpsilon(dms2def('01:46:20'), 0.1);
            expect(result.planets.saturn.degree).toBeWithinEpsilon(dms2def('01:45:17'), 0.1);
            expect(result.planets.rahu.degree).toBeWithinEpsilon(dms2def('26:22:55'), 0.1);
            expect(result.planets.ketu.degree).toBeWithinEpsilon(dms2def('26:22:55'), 0.1);
        });
        ...
        it('should calculate ' + D + ' nakshatras and padas correctly', () => {
            ...
            expect(result.ascendant.nakshatra).toBe('swati');
            ...
            expect(result.planets.mars.nakshatra).toBe('shravana');
            expect(result.planets.mars.nakshatraPada).toBe(2);
            expect(result.planets.mercury.nakshatra).toBe('purva_ashadha');
            expect(result.planets.mercury.nakshatraPada).toBe(1);
            expect(result.planets.jupiter.nakshatra).toBe('magha');
            expect(result.planets.jupiter.nakshatraPada).toBe(4);
            expect(result.planets.venus.nakshatra).toBe('uttara_phalguni');
            expect(result.planets.venus.nakshatraPada).toBe(2);
            expect(result.planets.saturn.nakshatra).toBe('chitra');
            expect(result.planets.saturn.nakshatraPada).toBe(3);
            expect(result.planets.rahu.nakshatra).toBe('purva_ashadha');
        ...
        it('should assign planets to correct houses in ' + D + ' chart', () => {
            ...
            expect(result.planets.sun.house).toBe(7);
            expect(result.planets.moon.house).toBe(11);
            expect(result.planets.mars.house).toBe(4);
            ...
            expect(result.planets.rahu.house).toBe(3);
