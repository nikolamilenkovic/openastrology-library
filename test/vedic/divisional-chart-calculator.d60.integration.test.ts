...
describe(DivisionalChartCalculator.name, () => {
    ...
    describe(D + ' calculations for test person 1', () => {
        ...
        it('should calculate ' + D + ' planet signs successfully', () => {
            ...
            expect(result.planets.sun.sign).toBe('cancer');
            ...
            expect(result.planets.venus.sign).toBe('gemini');
        ...
        it('should calculate ' + D + ' ascendant degree correctly', () => {
            ...
            expect(result.ascendant.degree).toBeWithinEpsilon(dms2def('03:03:30'), 0.22);
        });

        it('should calculate ' + D + ' planet degrees correctly', () => {
            const result = chartCalculator.calculateDivisionalChart(rashiChart, D);

            expect(result.planets.sun.degree).toBeWithinEpsilon(dms2def('06:04:34'), 0.1);
            expect(result.planets.moon.degree).toBeWithinEpsilon(dms2def('25:20:13'), 0.54);
            expect(result.planets.mars.degree).toBeWithinEpsilon(dms2def('27:49:41'), 0.1);
            expect(result.planets.mercury.degree).toBeWithinEpsilon(dms2def('19:18:55'), 0.1);
            expect(result.planets.jupiter.degree).toBeWithinEpsilon(dms2def('12:26:32'), 0.1);
            expect(result.planets.venus.degree).toBeWithinEpsilon(dms2def('17:43:16'), 0.1);
            expect(result.planets.saturn.degree).toBeWithinEpsilon(dms2def('17:32:46'), 0.1);
            expect(result.planets.rahu.degree).toBeWithinEpsilon(dms2def('23:49:14'), 0.1);
            expect(result.planets.ketu.degree).toBeWithinEpsilon(dms2def('23:49:14'), 0.1);
        });
        ...
        it('should calculate ' + D + ' nakshatras and padas correctly', () => {
            ...
            expect(result.planets.sun.nakshatra).toBe('pushya');
            ...
            expect(result.planets.jupiter.nakshatra).toBe('shatabhisha');
            expect(result.planets.jupiter.nakshatraPada).toBe(2);
            expect(result.planets.venus.nakshatra).toBe('ardra');
            expect(result.planets.venus.nakshatraPada).toBe(4);
            expect(result.planets.saturn.nakshatra).toBe('shravana');
            expect(result.planets.saturn.nakshatraPada).toBe(3);
            expect(result.planets.rahu.nakshatra).toBe('mrigashira');
        ...
        it('should assign planets to correct houses in ' + D + ' chart', () => {
            ...
            expect(result.planets.mercury.house).toBe(5);
